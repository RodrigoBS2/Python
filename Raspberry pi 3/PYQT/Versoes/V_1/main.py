...
# --- O Novo Decodificador de Sincronia Instantânea ---
        ...
        try:
            if ser.in_waiting:
                ...
                while i < tamanho - 1:
                    ...
                    # O Byte 1 sempre tem que ser >= 128 (0x80)
                    if b1 >= 128:
                        b2 = buffer_serial[i+1]
                        
                        # O Byte 2 sempre tem que ser < 128
                        if b2 < 128:
                            # Remonta os 12 bits perfeitos
                            valor_adc = ((b1 & 0x7F) << 7) | b2
                            novas_tensoes.append((valor_adc / 4095.0) * 3.3)
                            i += 2
                        else:
                            # A porta pulou um byte de ruído. Avança apenas 1 casa para realinhar.
                            i += 1
                    else:
                        # Lixo no cabo. Ignora.
                        i += 1
                        
                # Apaga apenas o que já foi lido com sucesso
                del buffer_serial[:i]
...
class OsciloscopioApp(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Osciloscópio PyQtGraph - Protocolo Blindado")
        self.resize(850, 500) 
    ...
    def setup_ui(self):
        ...
        self.plot_widget = pg.PlotWidget(title="Leitura do Fotodiodo (Sincronizado via Bytes)")
        ...
        self.label_info = QtWidgets.QLabel("Tensão Média (DC): 0.000 V | Frequência: 0.0 Hz")
        ...
        self.slider_escala = self.criar_slider(10, 3300, int(self.escala_y * 1000), estilo_slider.replace('cyan', 'magenta'))
        ...
        self.slider_offset = self.criar_slider(0, 3200, int(self.offset_y * 1000), estilo_slider.replace('cyan', 'yellow'))
        ...
        self.slider_trigger = self.criar_slider(0, 3300, int(self.nivel_trigger * 1000), estilo_slider.replace('cyan', 'orange'))
