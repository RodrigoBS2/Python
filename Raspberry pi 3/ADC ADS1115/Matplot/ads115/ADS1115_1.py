...
import adafruit_ads1x15.ads1x15 as ads1x15  # Importação atualizada para a versão 3.0+

# --- Configuração do I2C e ADS1115 ---
try:
    i2c = busio.I2C(board.SCL, board.SDA)
    # data_rate=860 configura o chip para sua velocidade máxima
    ads = ADS.ADS1115(i2c, data_rate=860) 
    
    # O pino agora é referenciado através da nova classe Pin.A0
    chan = AnalogIn(ads, ads1x15.Pin.A0) 
    
except Exception as e:
    print(f"Erro ao inicializar o ADS1115: {e}")
    print("Verifique as conexões I2C e se o I2C está habilitado no raspi-config.")
    exit()
...
# Fila para transferir dados da Thread de leitura para a Thread do gráfico
...
def ler_adc_continuamente():
    # Tempo alvo entre leituras para 860 SPS
...
# Desenha as linhas guia do Trigger
...
def atualizar(frame):
    ...
    # Esvazia a fila preenchida pela Thread do ADC
    while not fila_dados.empty():
        tensao = fila_dados.get()
...
def ao_fechar(event):
    global rodando
    rodando = False # Sinaliza para a Thread parar
